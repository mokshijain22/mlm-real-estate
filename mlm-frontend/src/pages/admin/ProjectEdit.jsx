  ...
  useEffect(() => {
    api
      ...
      .then((res) => {
        ...
        setDefaultOwnerMinimum(p.defaultOwnerMinimum || "");
        setFacilities(p.facilities || []);
        setLandmarks(p.nearbyLandmarks || []);
        setBankIds((p.banks || []).map((b) => (typeof b === "string" ? b : b._id)));
      })
                  ...
                  {FACILITY_OPTIONS.map((f) => (
                    <div className="col-md-4 col-6" key={f}>
                      <div className="form-check mb-2">
                        ...
                        />
                        <label className="form-check-label" htmlFor={`facility-edit-${f}`}>
                          {f}
                        </label>
                      </div>
                    </div>
                    ...
                    {availableBanks.map((b) => (
                      <div className="col-md-4 col-6" key={b._id}>
                        <div className="form-check mb-2">
                          ...
                          />
                          <label className="form-check-label" htmlFor={`bank-edit-${b._id}`}>
                            {b.name}
                          </label>
                        </div>
                      </div>
